...
#Extract the first occurrence of StartDate (OUTPUT IS A SERIES)
gb = df.groupby(['StartDate', 'yyyy', 'mm', 'dd'])['StartDate'].first()
print(gb)

#Extract the first occurrence of StartDate (OUTPUT IS A DATAFRAME)
gb2=pd.DataFrame({'first' : df.groupby(['StartDate', 'yyyy', 'mm', 'dd'])['StartDate'].first()}).reset_index()
gb2.info()
